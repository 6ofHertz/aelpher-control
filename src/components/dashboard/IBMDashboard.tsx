...
import { Briefcase, Clock, Target, TrendingUp, Plus, ChevronRight } from 'lucide-react';
...
export function IBMDashboard() {
  ...
  const nba = useNextBestAction('ibm');
  const nbaParent = nba ? topics.find(t => t.id === nba.topic_id) : null;
  const { logs } = useActivityLogs('ibm', 10);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-foreground">IBM Internship</h1>
          ...
          <div className="flex flex-col rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-2 flex items-baseline gap-1">
              <span className="font-mono text-3xl font-bold text-primary">
                {metrics.ibm.completedSubtopics}
              </span>
              <span className="font-mono text-sm text-muted-foreground">items</span>
            </div>
          </div>
          ...
          <div className="flex flex-col rounded-2xl border border-border bg-card p-5 shadow-sm">
            ...
            <div className="mt-2 flex items-baseline gap-1">
              <span className="font-mono text-3xl font-bold text-foreground">
                {metrics.ibm.totalSubtopics - metrics.ibm.completedSubtopics}
              </span>
              <span className="font-mono text-sm text-muted-foreground">items</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-3 gap-6">
        ...
        <div className="col-span-2 space-y-4">
          ...
          {topics.length === 0 ? (
            <EmptyState type="topics" armType="ibm" />
          ) : (
